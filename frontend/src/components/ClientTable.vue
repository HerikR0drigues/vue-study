...
<template>
    <div class="w-full h-full relative font-sofia">
        ...
        <div class="p-6 h-full bg-aawzBlack rounded-2xl shadow-lg border-2 border-aawzMain flex flex-col overflow-y-auto">
            <h2 class="text-2xl font-bold text-aawzMain mb-8 text-center">Tabela de Clientes</h2>
            <div class="space-y-2 text-center" v-if="Object.keys(clientes).length <= 0">
                <p class="text-xl text-gray-300 font-bold">Nenhum cliente cadastrado!</p>
            </div>
            <div v-else class="space-y-2 overflow-y-auto max-h-[600px] scrollbar-thumb-aawzMain scrollbar-track-aawzBlack scrollbar-thin px-2">
                ...
                <div v-for="(cliente, index) in clientes" :key="index"
                    ...
                    <font-awesome-icon :icon="['fas', 'eye']" class="cursor-pointer absolute right-0 top-2 mr-14 text-blue-600 hover:opacity-60" @click="toggleExpand(index)"/>
                    <font-awesome-icon :icon="['fas', 'pen-to-square']" class="cursor-pointer absolute right-0 top-2 mr-8 text-yellow-300 hover:opacity-60" @click="handleEdit(cliente)"/>
                    <font-awesome-icon :icon="['fas', 'trash-can']" class="cursor-pointer absolute right-0 top-2 mr-2 text-red-500 hover:opacity-60" @click="handleDelete(cliente)"/>
